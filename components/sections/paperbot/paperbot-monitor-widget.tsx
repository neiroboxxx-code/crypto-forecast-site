...
type Props = { monitor: PaperBotMonitor | null };
...
function StatusDot({ lastAt }: { lastAt: string | null }) {
    if (!lastAt) {
...
export function PaperbotMonitorWidget({ monitor }: Props) {
        ...
        >
            <div className="flex flex-col gap-3">
                ...
                <div className="flex items-center gap-3 rounded-xl border border-white/8 bg-black/20 px-4 py-3">
                    <StatusDot lastAt={lastAt} />
                    <div className="flex-1 min-w-0">
                        <div className="text-[12px] font-semibold text-white/85">
                            {alive ? "Работает" : lastAt ? "Нет данных" : "Ожидает первого тика"}
                        </div>
