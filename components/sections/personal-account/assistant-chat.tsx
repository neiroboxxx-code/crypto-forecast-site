...
export function AssistantChat() {
    ...
    const commitRename = () => {
        ...
        const title = renameDraft.trim() || "Новый чат";
        setThreads((prev) =>
            prev.map((x) => (x.id === renamingThreadId ? { ...x, title, updatedAt: nowIso() } : x)),
        );
        cancelRename();
    };
    ...
    const deleteThread = useCallback(
        (tid: string) => {
            if (
                !window.confirm(
                    "Удалить этот чат? Все сообщения в нём будут удалены с этого устройства.",
                )
            ) {
                return;
            }
            ...
            if (next.length === 0) {
                const nid = crypto.randomUUID();
                setThreads([{ id: nid, title: "Новый чат", messages: [], updatedAt: nowIso() }]);
                setActiveThreadId(nid);
            } else {
                setThreads(next);
                ...
                <div className="flex max-h-40 flex-row gap-1.5 overflow-x-auto pb-1 md:max-h-none md:flex-col md:overflow-y-auto md:pr-1">
                    {[...threads]
                        ...
                        .map((t) => {
                            ...
                            const renaming = renamingThreadId === t.id;
                            return (
                                <div
                                    key={t.id}
                                    className={`flex shrink-0 items-stretch gap-0.5 rounded-lg border md:w-full ${
                                        active
                                    ...
                                    {renaming ? (
                                        ...
                                        >
                                            <input
                                                ...
                                                className="w-full rounded-md border border-white/15 bg-black/40 px-2 py-1.5 text-[12px] text-white outline-none focus:border-fuchsia-400/40"
                                                autoFocus
                                                maxLength={80}
                                            ...
                                            <div className="flex gap-1.5">
                                                <button
                                                    type="submit"
                                                    className="rounded-md border border-fuchsia-400/35 bg-fuchsia-400/15 px-2 py-1 text-[11px] font-semibold text-fuchsia-50"
                                                >
                                                    OK
                                                </button>
                                                <button
                                                    ...
                                                    className="rounded-md border border-white/12 px-2 py-1 text-[11px] text-white/65"
                                                >
                                                    Отмена
                                                </button>
                                            </div>
                                        ...
                                        <>
                                            <button
                                                ...
                                                className="min-w-0 flex-1 px-2.5 py-2 text-left text-[12px] leading-snug transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-fuchsia-400/30"
                                            >
                                                <span className="line-clamp-2 break-words">{t.title || "Диалог"}</span>
                                            </button>
                                            <div className="flex shrink-0 flex-col justify-center gap-0.5 border-l border-white/10 py-1 pr-1 pl-0.5">
                                                <button
                                                    type="button"
                                                    title="Переименовать"
                                                    ...
                                                    className="rounded p-1 text-white/45 transition hover:bg-white/10 hover:text-fuchsia-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-fuchsia-400/35"
                                                >
                                                ...
                                                <button
                                                    ...
                                                    className="rounded p-1 text-white/45 transition hover:bg-rose-500/15 hover:text-rose-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-400/35"
                                                >
